...
class spsSincronizacao {
  ...
  static Future<bool> sincronizarQuestionarios() async {
    ...
    var dadosSincronizacaoAtual =
        await objSpsDaoSincronizacao.listaDadosSincronizacao();
...
//UPLOAD - SINCRONIZAÇÃO - Rotina de atualização Local(Sqlite) para o servidor(Rest API) - Cabeçalhos, Itens e anexos
      ...
      if (resultLista.length > 0) {
        if (jaNotificado == 0) {
          await spsNotificacao.notificarInicioProgressoIndeterminado(0,
              'SPS-Schuler Production System', 'Sincronização de Dados', flip);
          jaNotificado = 1;
        }
      }
      ...
      await Future.forEach(resultLista, (questionario) async {
        ...
        if (sincLista == 0) {
          ...
          print(
              '=== === UPLOAD - Update dados de Referência. Código programação: ' +
                  questionario["codigo_programacao"].toString());
        }
...
//      }
      ...
      if (result.length > 0) {
        if (jaNotificado == 0) {
          await spsNotificacao.notificarInicioProgressoIndeterminado(
              0,
              'SPS - Schuler Production System',
      ...
      await Future.forEach(result, (item) async {
        ...
        //Atualizar registro no PostgreSQL (via API REST) SAVE_RESPOSTA tipo checklist
        final atualizacaoStatusResposta =
        ...
        if (sincItem == 0) {
          ...
          objQuestionarioItemDao.updateQuestionarioItemSincronizacao(
              ...
              item["item_checklist"].toString());
          print(
              '=== === UPLOAD - Update dados de Item, comentários, aprovação e respostas. Codigo_programação: ' +
                  item["codigo_programacao"].toString() +
                  ', item: ' +
                  item["item_checklist"].toString());
        }
...
//      }
      ...
      debugPrint(
          "=== UPLOAD - INICIO SINCRONIZAÇÃO DE DADOS (Tabela: checklist_resp_multipla) =============================================");
      //Ler dados não sincronizados do SQlite
      final List<Map<String, dynamic>> result_mult =
          await objQuestionarioRespMultiplaDao
              .selectSincronizacaoRespMultipla();
      var _wregistros_mult = result_mult.length;
      if (result_mult.length > 0) {
        if (jaNotificado == 0) {
          await spsNotificacao.notificarInicioProgressoIndeterminado(
              0,
              'SPS - Schuler Production System',
      ...
      //debugPrint("Ler dados não sincronizados do SQlite (quantidade de registro: " +_wregistros.toString() +")");
      var windex_mult = 0;
      await Future.forEach(result_mult, (item) async {
        var _wsincronizado = "";
        var sincMult = 0;
        //Atualizar registro no PostgreSQL (via API REST) campo subcodigo_resposta e texto_adicional (salvar/deletar)
        final SpsHttpQuestionarioRespMultipla objQuestionarioRespMultiplaHttp =
            SpsHttpQuestionarioRespMultipla();
        final atualizacaoRespMultipla =
            await objQuestionarioRespMultiplaHttp.QuestionarioSaveRespMultipla(
                item["codigo_empresa"],
                item["codigo_programacao"].toString(),
                item["registro_colaborador"],
                item["identificacao_utilizador"],
                ...
                usuarioAtual.tipo == "INTERNO" ||
                    ...
                    ? usuarioAtual.registro_usuario
                    : usuarioAtual.codigo_usuario,
                item["sincronizado"] == "N" ? "SALVAR" : "DELETAR");
        if (atualizacaoRespMultipla != true) {
          sincMult = 1;
          //debugPrint("ERRO => Dados do registro não sincronizado: " + item.toString());
        }
        ...
        if (sincMult == 0) {
          //print('Iniciando a limpeza dos itens a serem sincronizados');
          objQuestionarioRespMultiplaDao
              .updateQuestionarioRespMultiplaSincronizacao(
                  item["codigo_empresa"],
                  item["codigo_programacao"].toString(),
                  item["registro_colaborador"],
                  item["identificacao_utilizador"],
          ...
          print(
              '=== === UPLOAD - Update dados de resposta multipla. Codigo_programação: ' +
                  item["codigo_programacao"].toString() +
                  ', item: ' +
                  item["item_checklist"].toString() +
        ...
        windex_mult = windex_mult + 1;
      });
//      while (windex < _wregistros) {
//      }
      debugPrint(
          "=== UPLOAD - FIM INICIO SINCRONIZAÇÃO DE DADOS (Tabela: checklist_resp_multipla) =============================================");
      ...
      await Future.forEach(resultMidia, (midia) async {
        ...
        dadosArquivo = {
          ...
          'identificacao_utilizador':
              midia['identificacao_utilizador'].toString(),
          'item_checklist': midia['item_checklist'].toString(),
          'item_anexo': midia['item_anexo'].toString(),
          'nome_arquivo': midia['nome_arquivo'].toString(),
          'titulo_arquivo': midia["titulo_arquivo"].toString(),
          'usuresponsavel': midia['usuresponsavel'].toString(),
          'dthratualizacao': midia['dthratualizacao'].toString(),
        ...
        if (retorno1Midia == "2") {
          ...
          if (midia['sincronizado'].toString() == "T" ||
            ...
            FormData formData = FormData.fromMap({
              "files": [
                await MultipartFile.fromFile(
                    ...
                    filename: midia['nome_arquivo'].toString()),
              ],
              "codigo_programacao": midia["codigo_programacao"].toString(),
            ...
            objSpsDaoQuestionarioMidia.updateQuestionarioMidiaSincronizacao(
                ...
                midia["item_checklist"].toString(),
                midia["item_anexo"].toString());
            print('=== === UPLOAD - Update de anexo. Codigo_programação: ' +
                midia["codigo_programacao"].toString() +
                ', item: ' +
                midia["item_checklist"].toString() +
                ', anexo: ' +
...
//DOWNLOAD - SINCRONIZAÇÃO - Rotina de atualização servidor(Rest API) para Local(Sqlite) - Anexos (to be done: Cabeçalhos, Itens)
      debugPrint(
          "DOWNLOAD - SINCRONIZAÇÃO - Rotina de atualização servidor(Rest API) para Local(Sqlite) - Anexos (to be done: Cabeçalhos, Itens) =============================================");
      jaNotificado = await sincronizarAnexosServerToLocal(
          'EXTERNO', 'CONTROLE DE QUALIDADE', flip, jaNotificado);
      jaNotificado = await sincronizarAnexosServerToLocal(
          'INTERNO', 'CONTROLE DE QUALIDADE', flip, jaNotificado);
      jaNotificado = await sincronizarAnexosServerToLocal(
          'EXTERNO', 'CHECKLIST', flip, jaNotificado);
      jaNotificado = await sincronizarAnexosServerToLocal(
          'INTERNO', 'CHECKLIST', flip, jaNotificado);
      jaNotificado = await sincronizarAnexosServerToLocal(
          'EXTERNO', 'PESQUISA', flip, jaNotificado);
      jaNotificado = await sincronizarAnexosServerToLocal(
          'INTERNO', 'PESQUISA', flip, jaNotificado);
      debugPrint(
...
//DOWNLOAD - SINCRONIZAÇÃO - Rotina de atualização servidor(Rest API) para Local(Sqlite) - Anexos (to be done: Cabeçalhos, Itens)
  ...
  static Future<int> sincronizarAnexosServerToLocal(
      _origemUsuario, _tipoChecklist, flip, jaNotificado) async {
    ...
    if (dadosQuestionario != null) {
      ...
      await Future.forEach(dadosQuestionario, (questionario) async {
        dadosArquivo = {
          ...
          'codigo_programacao': questionario["codigo_programacao"].toString(),
          'registro_colaborador': questionario["registro_colaborador"],
          'identificacao_utilizador':
              questionario['identificacao_utilizador'].toString(),
        };
        ...
        final int limparTabeLaLOCAL = await objSpsDaoQuestionarioMidia
            .emptyTable(questionario["codigo_programacao"].toString());
        ...
        if (ResultadoSave == 1) {
          ...
          await Future.forEach(dadosDeAnexosServidor, (AnexoServidor) async {
            String path = usuarioAtual.document_root_folder +
                '/' +
                AnexoServidor["nome_arquivo"].toString();
...
//              print('Arquivo da web ===>'+AnexoServidor["nome_arquivo"].toString());
              if (AnexoServidor["nome_arquivo"].toString() != null &&
                  AnexoServidor["nome_arquivo"].toString() != "null") {
                String ArquivoParaDownload =
                    'https://10.17.20.45/CHECKLIST/ANEXOS/' +
                        AnexoServidor["codigo_programacao"].toString() +
                        '_' +
                        '_' +
                        AnexoServidor["identificacao_utilizador"].toString() +
                        '_' +
                        AnexoServidor["item_checklist"].toString() +
                        '/' +
                        AnexoServidor["nome_arquivo"].toString();
                String destinoLocal =
                    usuarioAtual.document_root_folder.toString() +
                ...
                print('baixar ==> ' +
                    ...
                    ' Para ' +
                    destinoLocal.toString());
                await objspsUpDown
                    .downloadQuestionarioMidia(
                        ArquivoParaDownload, destinoLocal)
                    .then((String statusDownload) async {
                  if (statusDownload == '1') {
                    ...
                    if (tipoArquivo == 'mp4' || tipoArquivo == 'MP4') {
                      ...
                      spsMidiaUtils
                          .criarVideoThumb(fileList: _listaArquivos)
                          .then((value) => null);
                    ...
                    if (statusDownload == '404') {
                      ...
                      dadosApagarArquivoComErro = {
                        ...
                        'codigo_programacao':
                            AnexoServidor["codigo_programacao"].toString(),
                        'registro_colaborador':
                            AnexoServidor["registro_colaborador"],
                        'identificacao_utilizador':
                            AnexoServidor['identificacao_utilizador']
                                .toString(),
                        'item_checklist':
                            AnexoServidor['item_checklist'].toString(),
                        ...
                        'nome_arquivo':
                            AnexoServidor['nome_arquivo'].toString(),
                        'titulo_arquivo':
                            AnexoServidor["titulo_arquivo"].toString(),
                        'usuresponsavel':
                            AnexoServidor['usuresponsavel'].toString(),
                        'dthratualizacao':
                            AnexoServidor['dthratualizacao'].toString(),
              ...
              print(
                  'Não foi necessário baixar o arquivo pois ja existe no dispositivo ==> ' +
                      AnexoServidor["nome_arquivo"].toString());
            }
            ;
